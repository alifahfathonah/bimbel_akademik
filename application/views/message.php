<?php if ($this->session->has_userdata('gagal')) { ?>

    <div class="alert alert-danger alert-dismissible show fade mt-2">
        <!-- <i class="fas fa-exclamation-triangle"></i> -->
        <span><strong>Ada yang salah!</strong> <?= $this->session->flashdata('gagal'); ?></span>
        <button class="close" data-dismiss="alert">
            <span>&times;</span>
        </button>
    </div>


<?php } else if ($this->session->has_userdata('sukses')) { ?>

    <div class="alert alert-success alert-dismissible show fade mt-2">
        <!-- <i class="fas fa-exclamation-triangle"></i> -->
        <span><strong>Sukses</strong> <?= $this->session->flashdata('sukses'); ?></span>
        <button class="close" data-dismiss="alert">
            <span>&times;</span>
        </button>
    </div>
<?php } else if ($this->session->has_userdata('daftar')) { ?>
    <span class="row">
        <span class="col-12 col-md-6 col-lg-6">
            <span class="alert alert-success alert-has-icon alert-dismissible show fade">
                <div class="alert-icon"><i class="far fa-check-circle"></i></div>
                <div class="alert-body">
                    <div class="alert-title"><?= $this->session->flashdata('daftar'); ?></div>
                    Silahkan <a data-toggle="modal" data-target="#modal-5">Login</a>
                </div>
                <button class="close" data-dismiss="alert">
                    <span>&times;</span>
                </button>
            </span>
        </span>
    </span>

<?php } else if ($this->session->has_userdata('gagal_load')) { ?>
    <div class="row">
        <div class="col-6">
            <div class="alert alert-danger alert-dismissible show fade mt-2">
                <!-- <i class="fas fa-exclamation-triangle"></i> -->
                <?= $this->session->flashdata('gagal_load'); ?></span><a href="<?= base_url('admin/penjadwalan') ?>">Silahkan masukkan mapel</a>
                <button class="close" data-dismiss="alert">
                    <span>&times;</span>
                </button>
            </div>
        </div>
    </div>

<?php } ?>